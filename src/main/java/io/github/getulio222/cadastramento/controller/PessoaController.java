...
public class PessoaController {
    ...
    @PostMapping
    public ResponseEntity<PessoaDTOResponse> salvar(
            @RequestBody @Valid PessoaDTORequest dto) {

        Pessoa pessoa = pessoaService.salvar(dto);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new PessoaDTOResponse(
                        pessoa.getId(),
                        pessoa.getNome(),
                        pessoa.getCpf(),
                        pessoa.getCidade(),
    ...
    @GetMapping("/nome")
    public ResponseEntity<List<PessoaDTOResponse>> buscarPorNome(
